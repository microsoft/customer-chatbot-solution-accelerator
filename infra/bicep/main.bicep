...
resource existingAiFoundryAiServices 'Microsoft.CognitiveServices/accounts@2026-01-15-preview' existing = if (useExistingAiFoundryAiProject) {
  name: aiFoundryAiServicesResourceName
  scope: resourceGroup(aiFoundryAiServicesSubscriptionId, aiFoundryAiServicesResourceGroupName)
}

module existingAiFoundryAiServicesDeployments 'modules/ai/ai-foundry-model-deployment.bicep' = if (useExistingAiFoundryAiProject) {
...
module aiFoundryAiServices 'br:mcr.microsoft.com/bicep/avm/res/cognitive-services/account:0.14.2' = if (!useExistingAiFoundryAiProject) {
...
resource existingAiFoundryAiServicesProject 'Microsoft.CognitiveServices/accounts/projects@2026-01-15-preview' existing = if (useExistingAiFoundryAiProject) {
...
module webSiteBackend 'modules/compute/app-service.bicep' = {
...
module backendToAiProjectUserRole 'modules/identity/role-assignments.bicep' = if (!useExistingAiFoundryAiProject) {
...
module backendToExistingAiProjectUserRole 'modules/identity/role-assignments.bicep' = if (useExistingAiFoundryAiProject) {
...
module backendToSearchRole 'modules/identity/role-assignments.bicep' = {
...
module backendToExistingAiServicesUserRole 'modules/identity/role-assignments.bicep' = if (useExistingAiFoundryAiProject) {
...
module searchToExistingAiServicesOpenAIRole 'modules/identity/role-assignments.bicep' = if (useExistingAiFoundryAiProject) {
...
module webSite 'modules/compute/app-service.bicep' = {
  ...
  params: {
    ...
    // WAF aligned configuration for Private Networking
    outboundVnetRouting: {
      applicationTraffic: enablePrivateNetworking ? true : false
      imagePullTraffic: enablePrivateNetworking ? true : false
    }
    virtualNetworkSubnetId: enablePrivateNetworking ? virtualNetwork!.outputs.webserverfarmSubnetResourceId : null
