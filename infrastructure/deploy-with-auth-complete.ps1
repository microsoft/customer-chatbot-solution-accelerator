...
# This script handles the chicken-and-egg problem by creating the app registration
# with placeholder URLs first, then updating them after deployment
...
param(
    ...
    [switch]$CreateAppRegistration = $false
)
...
# Handle app registration creation or validation
if ($CreateAppRegistration -or (-not $AzureClientId)) {
    ...
    # Validate existing app registration
    Write-Host "`n🔍 Validating existing app registration..." -ForegroundColor Blue
    
    try {
        $appInfo = az ad app show --id $AzureClientId --query "{displayName: displayName, appId: appId}" -o json | ConvertFrom-Json
        Write-Host "✅ Found app registration: $($appInfo.displayName)" -ForegroundColor Green
    } catch {
        Write-Host "❌ App registration not found. Please check your Client ID or use -CreateAppRegistration" -ForegroundColor Red
        exit 1
    }
}
...
# Phase 4: Update App Registration with correct URLs
if ($CreateAppRegistration -or (-not $AzureClientId)) {
    Write-Host "`n🔧 PHASE 4: UPDATING APP REGISTRATION URLS" -ForegroundColor Magenta
    Write-Host "===========================================" -ForegroundColor Magenta
    
    Write-Host "Updating redirect URIs with actual frontend URL..." -ForegroundColor Blue
    
    # Update the app registration with correct URLs
    $authConfig = @{
        "web" = @{
...
if (-not $SkipIntegration) {
    Write-Host "`n🔍 PHASE 5: INTEGRATION TEST" -ForegroundColor Magenta
    Write-Host "============================" -ForegroundColor Magenta
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "❌ Phase 5 failed. Please check the integration test results." -ForegroundColor Red
    } else {
        Write-Host "`n✅ Phase 5 completed successfully!" -ForegroundColor Green
    }
} else {
    Write-Host "`n⏭️  Skipping Phase 5: Integration Test" -ForegroundColor Yellow
}
