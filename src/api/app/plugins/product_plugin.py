...
class ProductPlugin:
    ...
    @kernel_function(description="Lookup a product by ID and return natural language description")
    ...
    @kernel_function(description="Search products with hybrid AI Search + Cosmos DB for maximum speed and accuracy")
    def search(self, query: str, limit: int = 5) -> str:
        ...
        try:
            ...
            products = run_async_sync(cosmos_service.search_products_hybrid(query, limit))
            ...
            if len(products) == 1:
                ...
                if len(products) > 3:
                    response_parts.append(f"...and {len(products) - 3} more products available.")
    ...
    def search_fast(self, query: str, limit: int = 3) -> str:
        ...
        try:
            ...
            products = run_async_sync(cosmos_service.search_products_ai_search(query, limit))
    ...
    def _get_search_suggestions(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["blue", "red", "green", "white", "black", "color"]):
        ...
        elif any(word in query_lower for word in ["cheap", "expensive", "budget", "premium"]):
    ...
    @kernel_function(description="Get products by category with natural language responses")
    def get_by_category(self, category: str, limit: int = 5) -> str:
        """Get products by category with natural language response"""
        try:
            cosmos_service = get_cosmos_service()
            products = run_async_sync(cosmos_service.get_products_by_category(category, limit))
    ...
    @kernel_function(description="Get all available products with natural language response")
    def get_all_products(self, limit: int = 10) -> str:
        ...
        try:
            ...
            response_parts = [f"We offer {len(products)} products across different categories:"]
