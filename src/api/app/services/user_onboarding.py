...
async def create_demo_order_history(user_id: str) -> List[Transaction]:
    ...
    logger.info(f"Creating demo order history for new user: {user_id} by replicating sample user orders")
    ...
    for sample_user_id in SAMPLE_USER_IDS:
        try:
            sample_orders = await cosmos_service.get_orders_by_customer(sample_user_id, limit=50)
            all_sample_orders.extend(sample_orders)
            logger.info(f"Found {len(sample_orders)} orders for sample user {sample_user_id}")
    ...
    for idx, sample_order in enumerate(selected_orders, start=1):
        try:
            ...
            transaction = Transaction(
                ...
                shipping_address=sample_order.get("shipping_address", {
                ...
                created_at=datetime.utcnow() - timedelta(days=random.randint(1, 180)),  # Random date within last 6 months
    ...
    logger.info(f"Successfully created {len(demo_orders)} replicated orders for user {user_id}")
    return demo_orders
...
async def create_fallback_demo_orders(user_id: str) -> List[Transaction]:
    ...
    logger.info(f"Successfully created {len(demo_orders)} fallback demo orders for user {user_id}")
    return demo_orders
