...
async def get_product_categories():
    ...
    try:
        categories = await get_db_service().get_product_categories()
        return categories
    except Exception as e:
        raise HTTPException(
...
async def get_featured_products(limit: int = Query(10, ge=1, le=50)):
    ...
    try:
        products = await get_db_service().get_featured_products(limit)
        return products
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching featured products: {str(e)}"
        )
...
async def get_related_products(product_id: str, limit: int = Query(5, ge=1, le=20)):
    """Get products related to the specified product"""
    try:
        related_products = await get_db_service().get_related_products(product_id, limit)
        return related_products
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error fetching related products: {str(e)}"
        )